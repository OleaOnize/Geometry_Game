class Game {
    ...
    start() {
      //this.player = new Player(this.ctx, 400, this.canvas.height -180, 80, 80); //PREGUNTAR SI ESTÁ BIEN
      if (this.obs.length === 0) {
        this.obs = obstacles.map((obsData) => {
          const { x, y, width, height, type } = obsData;
          return new Obstacle(this.ctx, this.ctx.canvas.width + x, y, width, height, type);
        })
      }
    ...
    this.ctx.fillText(`SCORE:  ${this.scoreboard}` + ` %`, this.canvas.width / 2, this.canvas.height / 2 +200);
    this.obs = [];
